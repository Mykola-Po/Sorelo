...
describe("Graphology Graph Builder", () => {
  ...
  it("adds ghost nodes with ghost-specific attributes", () => {
    const snapshot: GraphSnapshot = {
      revision: 1,
      counts: { conceptCount: 1, linkCount: 0 },
      concepts: [
        {
          id: "concept-1",
          title: "Observed Concept",
          conceptType: "fact",
          summary: "Observed",
          description: null,
          x: 120,
    ...
    const graph = buildGraphologyInstance(snapshot, undefined, [
      {
        id: "ghost-1",
        title: "Ghost Concept",
        conceptType: "belief",
    ...
    expect(graph.order).toBe(2);
    expect(graph.hasNode("ghost-1")).toBe(true);

    const ghostAttributes = graph.getNodeAttributes("ghost-1");
    expect(ghostAttributes.label).toBe("Ghost Concept (Ghost)");
    expect(ghostAttributes.isGhost).toBe(true);
    expect(ghostAttributes.size).toBe(16);
    expect(ghostAttributes.color).toBe("rgba(134, 142, 150, 0.3)");
  });

  it("prefers canonical Concept nodes over ghosts with the same id", () => {
    const snapshot: GraphSnapshot = {
      revision: 3,
      counts: { conceptCount: 1, linkCount: 0 },
      concepts: [
    ...
    const nodeAttributes = graph.getNodeAttributes("shared-id");
    expect(nodeAttributes.label).toBe("Canonical Concept");
    expect(nodeAttributes.conceptType).toBe("trigger");
    expect(nodeAttributes.isGhost).toBeUndefined();
    expect(nodeAttributes.x).toBe(220);
    expect(nodeAttributes.y).toBe(260);
  });
});
